...
- 輸入單字或整句漢字，顯示帶聲調的拼音與要打的字母（例如 `銀行` → `yínháng` / `yinhang`）
- 依詞組判斷多音字：`銀行` 讀 háng、`重新開始` 讀 chóng，不會逐字亂猜
- 逐字對照卡：漢字、拼音、鍵盤字母、聲調；點一下可看該字的其他讀音（罕用讀音會標示出來）
- `ü` 會提醒在鍵盤上要打 `v`（`綠` lǜ → 打 `lv`）
- 一鍵複製整句拼音或整串字母
- 最近查詢記錄，存在手機本機
- 繁體、簡體都能查
...
字音來自 pypinyin 與 CC-CEDICT，採大陸普通話標準（`頭髮` 作 `tóufa`、`個` 作 `ge`）。
這正好是 iOS 拼音鍵盤所依循的標準，所以拿來查「要打什麼字母」是準的；
但若要對照臺灣教育部審定音，會有少數字不同。
